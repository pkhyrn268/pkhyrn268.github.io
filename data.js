...
const FEATURED_PROJECTS = [
  {
    ...
    desc: '80+ GPU · 15대 서버를 운영하는 환경에서, 서버 관리자 관점의 계정·접속·스토리지·포트 관리 절차를 Kubernetes 자동화로 묶었다. 사람이 SSH로 들어가 처리하던 생성·복구·회수 흐름을 config-server 하나로 수렴시켰다.',
    oneLiner: '80+ GPU · 15대 서버 규모 AI 인프라에서 계정, NodePort, 스토리지, 자원 회수까지 수명 주기를 자동화한 config-server',
    problem: '동국대 AI 연구실 GPU 서버실은 다수 사용자가 동시에 학습·실험을 돌리는 환경이다. 계정 생성·삭제, 외부 SSH/Jupyter 접속 정보 배정, 홈 디렉터리 준비, Pod 배치와 회수를 관리자가 손으로 처리하다 보니 요청이 밀리면 그대로 병목이 됐고, 실제 클러스터 상태와 관리 기록이 어긋나면 장애 원인을 찾는 데도 시간이 오래 걸렸다.',
    ...
    bullets: [
      ...
      'NodePort를 새로 배정하기 직전, DB 기록만 믿지 않고 클러스터 전체 Service 목록과 대조해 실제 사용 중인 포트를 다시 수집한다. 이 reconcile 단계로 중복 배정과 유령 포트 누적을 막고, 삭제 실패 뒤 남은 포트도 다시 회수할 수 있게 했다',
      ...
      '사용자 계정 생성 뒤 NAS 홈 디렉터리 숫자 소유권과 권한, AD 계정·그룹, Pod·Service·NodePort 응답값까지 순서대로 검증하는 운영 체크리스트를 코드와 매뉴얼에 함께 반영',
    ...
    troubleshooting: [
      ...
      {
        problem: '관리 기록에는 포트가 사용 중이라고 남아 있는데 실제 Kubernetes Service는 이미 사라진 상태가 누적되면, 비어 있는 NodePort가 있어도 새 Pod 생성이 막히는 정합성 문제가 발생할 수 있었음',
        solution: 'NodePort 할당 전에 클러스터 실제 Service 목록과 infra-mysql 기록을 다시 맞추는 reconcile 절차를 넣고, 삭제 경로에서도 Pod·Service·NodePort 배정 기록·Kerberos 정리 순서를 명시적으로 검증하도록 보강했다.',
      },
    ],
    results: [
      '계정 발급 → 만료 회수까지 관리자 개입 없이 동작하는 자동화 파이프라인 완성',
      '실제 클러스터 상태를 기준으로 NodePort·UID/GID·계정 정합성을 복구하는 운영-safe 자동화 구조 확립',
  ...
  },
  {
    ...
    desc: '컨테이너 이미지, 모니터링, 원격 부팅, 사용자 수명 주기, Kerberos/NFS 운영까지 — 관리자 홈 디렉토리에 흩어져 있던 서버실 운영 자산 전체를 단일 레포, 매뉴얼, systemd/Ansible 자동화로 옮겼다.',
    oneLiner: 'DECS 서버 운영의 통합 레포 — GPU 런타임 표준화, 모니터링, 원격 부팅, 사용자 수명 주기, Kerberos/NFS 운영을 코드로 관리',
    problem: '서버실 운영 지식이 관리자 개인 홈 디렉토리(~/decs, ~/k8s, ~/uid, ~/krb …)에 흩어져 있으면 장애 대응이 특정 사람의 기억에 의존하게 된다. GPU 드라이버, Docker runtime, NFS mount, SSH 접속 경로, 전원 복구 절차처럼 서버 관리자에게 중요한 설정이 서버마다 달라지면, 같은 증상도 매번 다른 방식으로 고쳐야 했다. 운영 스크립트와 설정을 subtree 히스토리째 하나의 레포로 모았다.',
    ...
    bullets: [
      ...
      'Ubuntu 22.04 재설치 뒤 반복되던 `Failed to initialize NVML: Unknown Error`를 운영 이슈로 추적해, `/etc/docker/daemon.json`의 `nvidia` runtime 등록, Docker cgroup driver `cgroupfs`, `nvidia-persistenced` enable/run을 표준 설정으로 정리하고 매뉴얼화',
      ...
      'NFS mount 상태와 버전을 확인하는 체크 스크립트, GPU/SSH 상태 Prometheus alert, 외부 접속 가용성 확인 로직을 붙여 GPU·스토리지·접속 장애를 사용자 신고 전에 먼저 감지하도록 운영 관제 체계 구성',
      ...
      'Ansible로 Docker 버전, 자체 systemd 서비스, 운영 패키지 설치를 서버별로 표준화하고, 예외 서버와 점검 체크리스트까지 문서화해 "어떤 서버가 왜 다른지"를 코드와 문서에서 동시에 추적 가능하게 정리',
    ],
    troubleshooting: [
      {
      ...
      {
        problem: 'OS 재설치 이후 여러 서버에서 컨테이너 내부 `nvidia-smi`가 간헐적으로 실패하고, 장시간 미사용 뒤 `Failed to initialize NVML: Unknown Error`가 반복 발생',
        solution: 'Docker runtime/cgroup 설정, NVIDIA toolkit/CUDA 버전, `nvidia-persistenced` 상태를 함께 비교해 공통 원인을 좁혔다. 이후 `daemon.json` 표준 설정, 서비스 자동 시작, 모듈 reload 및 재부팅 기준, 점검 순서를 운영 매뉴얼과 서버 공통 설정으로 반영했다.',
      },
      {
        problem: '특정 서버에서 컨테이너 접속 지연과 패킷 유실이 반복되어 조사한 결과 `rx_missed_errors`가 크게 증가하고 있었음',
        solution: 'NIC ring buffer 초과 가능성을 보고 `RX descriptor queue` 크기를 512에서 4096으로 상향하고, 컨테이너 생성 시 `--init` 옵션을 적용해 zombie process 누적도 함께 줄였다. 이후 `rx_missed_errors: 0` 상태를 확인하며 재발 여부를 모니터링했다.',
      },
    ],
    results: [
      '흩어져 있던 운영 자산 전체를 단일 레포로 통합 — 관리자가 바뀌어도 이어지는 운영 체계',
      '서버 기동 → 헬스체크 → 컨테이너 복구 → 알림까지 부팅 전 과정 무인 자동화',
      'GPU 런타임, NFS mount, SSH 접속 경로, 운영 서비스 버전을 표준화해 서버별 설정 드리프트를 줄이는 기반 확보',
      'GPU 사용량을 실사용자 단위로 가시화한 Grafana 대시보드 — 자원 경합 추적 가능',
      'GPU·스토리지·접속 이상을 Prometheus·Grafana·Slack으로 선제 감지하고 복구 절차까지 연결한 운영 관제 체계 구축',
    ],
